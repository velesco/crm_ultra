...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Segment</h1>
            <p class="text-gray-600 dark:text-gray-400">Modify segment: {{ $segment->name }}</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('segments.show', $segment) }}" class="btn-secondary">
    ...
    <form method="POST" action="{{ route('segments.update', $segment) }}" class="space-y-6" x-data="segmentForm()">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" 
                        ...
                        <option value="static" {{ old('type', $segment->type) === 'static' ? 'selected' : '' }}>Static</option>
                        <option value="dynamic" {{ old('type', $segment->type) === 'dynamic' ? 'selected' : '' }}>Dynamic</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white @error('description') border-red-500 @enderror">{{ old('description', $segment->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div x-show="segmentType === 'dynamic'" 
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <div class="flex space-x-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="condition_logic" value="AND" class="text-blue-600" {{ old('condition_logic', $segment->condition_logic ?? 'AND') === 'AND' ? 'checked' : '' }}>
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">AND (all conditions must match)</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="condition_logic" value="OR" class="text-blue-600" {{ old('condition_logic', $segment->condition_logic ?? 'AND') === 'OR' ? 'checked' : '' }}>
                ...
                <div class="mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <h4 class="text-sm font-medium text-green-800 dark:text-green-200 mb-2">Current Performance</h4>
                    <div class="text-sm text-gray-700 dark:text-gray-300">
                        <p><strong>{{ number_format($segment->contacts_count ?? 0) }}</strong> contacts currently match this segment</p>
                        <p class="text-xs text-gray-500 mt-1">Last updated: {{ $segment->updated_at->format('M j, Y g:i A') }}</p>
                    </div>
                    <button type="button" 
                ...
                <div class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <h4 class="text-sm font-medium text-blue-800 dark:text-blue-200 mb-2">Preview Changes</h4>
                    <button type="button" 
        ...
        <div x-show="segmentType === 'static'" 
             x-transition
             class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Manage Contacts</h2>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Current Contacts -->
                <div>
                    <h3 class="text-md font-medium text-gray-900 dark:text-white mb-3">
                        Current Contacts ({{ $segment->contacts->count() }})
                    </h3>
                    <div class="border border-gray-300 dark:border-gray-600 rounded-lg p-3 h-64 overflow-y-auto">
                ...
                <!-- Add New Contacts -->
                <div>
                    <h3 class="text-md font-medium text-gray-900 dark:text-white mb-3">Add More Contacts</h3>
            ...
            <div class="space-y-4">
                ...
                <div x-show="segmentType === 'dynamic'" x-transition class="flex items-center">
                    ...
                    <input type="checkbox" 
                           ...
                           id="auto_refresh" 
                           value="1"
                           {{ old('auto_refresh', $segment->auto_refresh) ? 'checked' : '' }}
                           class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
        ...
        <div class="flex justify-between">
            <form method="POST" action="{{ route('segments.destroy', $segment) }}" 
                  class="inline"
                  onsubmit="return confirm('Are you sure you want to delete this segment? This action cannot be undone.')">
                @csrf
                @method('DELETE')
            ...
            <div class="flex space-x-4">
                <a href="{{ route('segments.show', $segment) }}" class="btn-secondary">Cancel</a>
...
function segmentForm() {
    return {
        segmentType: '{{ old('type', $segment->type) }}',
        previewResults: null
    }
}

function conditionsBuilder() {
    const existingConditions = @json(old('conditions', $segment->conditions ?? [{ 'field': '', 'operator': '', 'value': '' }]));
    ...
    return {
        ...
        previewConditions() {
            ...
            // Simulate API call
            setTimeout(() => {
                this.previewResults = "Changes would affect approximately 156 contacts";
            }, 1000);
        }
    }
}
